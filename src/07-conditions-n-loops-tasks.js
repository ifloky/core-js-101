...
function isInsideCircle(circle, point) {
  const distX = Math.sqrt((point.x - circle.center.x) ** 2);
  const distY = Math.sqrt((point.y - circle.center.y) ** 2);

  const diagonal = Math.sqrt(distX ** 2 + distY ** 2);
...
function isCreditCardNumber(ccn) {
  const ccnString = ccn.toString().replace(/\D/g, '');
